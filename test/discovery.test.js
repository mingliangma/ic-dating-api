...
const Hide = require('../server/model/hide');
    ...
    it('hide', done => {
      request(server)
        ...
        .end((err, res) => {
          ...
          request(server)
            .get(`/api/v1/discovery/myAccountId/${res.body.accountId}?latitude=43.7659095&longitude=-79.4141207&maxDistance=8000`)
            .end((err1, res1) => {
              expect(res1.statusCode).to.equal(200);
              expect(res1.body).to.have.property('totalItems');
              expect(res1.body.totalItems).to.equal(2);
              expect(res1.body.list).to.have.lengthOf(2);
              expect(res1.body.list[0]).to.have.property('accountId');
              request(server)
                .post(`/api/v1/discovery/hide/initiator/${res.body.accountId}/receiver/${res1.body.list[0].accountId}`)
                .end((err2, res2) => {
                  ...
                  request(server)
                    ...
                    .end((err3, res3) => {
                      ...
                      expect(res3.body).to.have.property('totalItems');
                      expect(res3.body.totalItems).to.equal(1);
                      expect(res3.body.list).to.have.lengthOf(1);
                      expect(res3.body.list[0].accountId).to.not.equal(res1.body.list[0].accountId);
                      done();
                    });
                });
            });
